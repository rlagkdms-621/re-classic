...
export default function Home() {
  ...
  function downloadResultImage() {
    ...
    link.download = `re-classic-${selectedArtwork?.title || "artwork"}-${selectedKeyword}.png`;
  ...
  return (
    <main className="min-h-screen bg-[#e7e2d8] px-6 py-10 text-[#1a1a1a] md:px-10 md:py-12">
      <div className="mx-auto max-w-7xl">
        <p className="mb-2 text-sm tracking-[0.4em] text-neutral-500">RE-CLASSIC</p>

        <h1 className="mb-4 text-5xl leading-tight md:text-7xl">현대를 입은 명작</h1>

        <p className="mb-1 text-lg text-neutral-700 md:text-xl">명화는 변하지 않았다.</p>
        <p className="mb-20 text-lg text-neutral-700 md:mb-24 md:text-xl">
          변한 것은 우리가 살아가는 시대였다.
        </p>
        ...
        <section className="mb-16">
          <button
            ...
            className="w-full rounded-[28px] border-2 border-black bg-black px-10 py-8 text-3xl font-semibold text-white transition hover:scale-[1.01]"
          >
        ...
        <section className="mb-24">
          <h2 className="mb-10 text-4xl md:text-5xl">샘플 명작 선택</h2>
          ...
          <div className="grid grid-cols-1 gap-8 md:grid-cols-2 xl:grid-cols-3">
            {sampleArtworks.map((artwork) => (
              <button
                ...
                className={`overflow-hidden rounded-[30px] border bg-white text-left transition-all duration-300 ${
                  selectedArtwork?.title === artwork.title
                    ? "scale-[1.02] border-black shadow-2xl"
              ...
              >
                ...
                <div className="p-6">
                  <h3 className="mb-2 text-2xl leading-snug">{artwork.title}</h3>
                  <p className="text-lg text-neutral-500">{artwork.artist}</p>
                </div>
              </button>
            ))}
          </div>
        ...
        <section className="mb-24">
          <h2 className="mb-8 text-4xl md:text-5xl">직접 업로드</h2>

          <label className="block cursor-pointer rounded-[30px] border-2 border-dashed border-neutral-500 bg-white/60 p-10 text-center transition hover:border-black hover:bg-white md:p-14">
            <input type="file" accept="image/*" onChange={handleUpload} className="hidden" />

            <p className="mb-3 text-3xl font-semibold">이미지를 업로드해주세요</p>
        ...
        <section className="mb-24">
          <h2 className="mb-8 text-4xl md:text-5xl">현대화 방향</h2>
          ...
          <div className="grid grid-cols-1 gap-5 md:grid-cols-3">
            {keywords.map((keyword) => (
              <button
                ...
                className={`rounded-[28px] border-2 px-12 py-8 text-3xl font-semibold transition-all duration-300 hover:scale-[1.02] ${
        ...
        <section className="mb-24 flex flex-wrap gap-6">
          <button
            ...
            className="rounded-[28px] bg-black px-12 py-7 text-2xl font-semibold text-white transition hover:scale-105 disabled:opacity-40"
          >
          ...
          {resultImage && (
            <button
              ...
              className="rounded-[28px] border-2 border-black bg-white px-12 py-7 text-2xl font-semibold text-black transition hover:scale-105 hover:bg-black hover:text-white"
            >
              결과 이미지 저장
            </button>
          )}
        ...
        <section className="grid grid-cols-1 gap-10 lg:grid-cols-2">
          ...
          <ArtworkPanel
            ...
            onImageClick={() => resultImage && openImageModal(resultImage, "현대화 결과")}
          />
        </section>

        {suitabilityResult && (
          <section className="mt-20 rounded-[40px] bg-white p-8 shadow-sm md:p-12">
            <p className="mb-4 text-sm tracking-[0.35em] text-neutral-400">
              PRE-CHECK REPORT
            </p>

            <h2 className="mb-10 text-4xl md:text-5xl">현대화 사전 적합도 분석</h2>

            <div
              className={`mb-10 rounded-[28px] border-2 p-8 ${
            ...
            >
              <p className="mb-3 text-4xl font-bold">{suitabilityResult.status}</p>
        ...
        {curatorReport && (
          <section className="mt-20 rounded-[40px] bg-white p-8 shadow-sm md:p-12">
            ...
            <h2 className="mb-10 text-4xl md:text-5xl">작품 분석</h2>
...
function ArchiveSection({
  ...
  return (
    <section className="mb-24 rounded-[40px] bg-[#111111] p-8 text-white shadow-sm md:p-12">
      <p className="mb-4 text-sm tracking-[0.35em] text-white/40">
        PUBLIC EXHIBITION ARCHIVE
      </p>

      <h2 className="mb-6 text-4xl md:text-6xl">다른 사용자의 변환 전시</h2>

      <p className="mb-12 max-w-4xl text-xl leading-9 text-white/60">
        이곳은 사용자들이 생성한 현대화 명작 결과가 자동으로 저장되는 전시 아카이브입니다.
        원작, 변환 이미지, 현대화 방향, 큐레이터 해설을 함께 감상할 수 있습니다.
      </p>

      {loading && <p className="text-2xl text-white/60">아카이브를 불러오는 중...</p>}

      {!loading && generations.length === 0 && (
        <p className="text-2xl text-white/60">아직 저장된 전시 결과가 없습니다.</p>
      )}

      <div className="grid gap-8 md:grid-cols-2">
        {generations.map((item) => (
          <div key={item.id} className="bg-white p-6 text-black">
            <div className="mb-5 flex flex-wrap items-end justify-between gap-4">
              <div>
                <p className="text-sm tracking-[0.25em] text-neutral-400">
                  {item.direction}
                </p>
                <h3 className="mt-2 text-3xl">{item.artwork_title}</h3>
            ...
            <div className="mt-6 border-t border-neutral-200 pt-6">
              ...
              <p className="line-clamp-4 text-lg leading-8 text-neutral-700">
...
function ArtworkPanel({
  ...
  return (
    <div className="rounded-[40px] bg-white p-8 shadow-sm md:p-10">
      <h2 className="mb-8 text-4xl md:text-6xl">{title}</h2>

      <div className="aspect-square overflow-hidden rounded-[30px] bg-neutral-100">
        {image ? (
          ...
          <div className="flex h-full w-full flex-col items-center justify-center text-2xl text-neutral-500">
            <div className="mb-6 h-10 w-10 animate-spin rounded-full border-2 border-neutral-300 border-t-black" />
            적합도 분석 후 생성 중...
          </div>
        ) : (
          <div className="flex h-full w-full items-center justify-center text-center text-2xl text-neutral-400">
            아직 작품이 생성되지 않았습니다.
          </div>
        )}
      </div>
...
function ImageModal({ image, title, onClose }: { image: string; title: string; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 p-8" onClick={onClose}>
      <div className="relative max-h-full max-w-6xl" onClick={(e) => e.stopPropagation()}>
        <button
          onClick={onClose}
          className="absolute right-0 top-[-64px] rounded-full border border-white/30 px-6 py-3 text-xl text-white transition hover:bg-white hover:text-black"
        >
          닫기
        </button>

        <p className="mb-4 text-2xl text-white">{title}</p>

        <img src={image} alt={title} className="max-h-[82vh] max-w-full rounded-[24px] object-contain shadow-2xl" />
      </div>
    </div>
  );
}
...
function ReportBlock({ title, content }: { title: string; content: string }) {
  return (
    <div className="border-t border-neutral-300 pt-6">
      <h3 className="mb-5 text-3xl">{title}</h3>
      <p className="whitespace-pre-wrap text-xl leading-[1.8] text-neutral-700 md:text-2xl">{content}</p>
    </div>
  );
}
...
function ComparisonTable({ rows }: { rows?: ComparisonRow[] }) {
  ...
  return (
    <div className="mt-16 overflow-hidden rounded-[28px] border border-neutral-200">
      <div className="bg-black px-8 py-6 text-white">
        <p className="text-sm tracking-[0.35em] text-white/50">STRUCTURAL COMPARISON</p>
        <h3 className="mt-3 text-4xl">변환 근거 비교표</h3>
      </div>
...
function TableCell({ label, content }: { label: string; content: string }) {
  return (
    <div className="border-b border-r border-neutral-200 p-6 md:border-b-0">
      <p className="mb-4 text-xs tracking-[0.25em] text-neutral-400">{label}</p>
      <p className="text-lg leading-8 text-neutral-700">{content}</p>
    </div>
  );
}
...
function PromptPanel({ prompt }: { prompt: string }) {
  return (
    <section className="mt-20 rounded-[40px] bg-[#111111] p-8 text-white shadow-sm md:p-12">
      <p className="mb-4 text-sm tracking-[0.35em] text-white/40">PROMPT DESIGN DOCUMENTATION</p>
      <h2 className="mb-8 text-4xl md:text-5xl">AI에게 전달된 창작 지시문</h2>

      <p className="mb-8 max-w-4xl text-xl leading-9 text-white/60">
        이 패널은 이미지가 어떤 프롬프트 설계를 통해 생성되었는지 보여줍니다.
      </p>

      <pre className="max-h-[520px] overflow-auto whitespace-pre-wrap rounded-[28px] bg-white/10 p-8 text-lg leading-8 text-white/80">
        {prompt}
      </pre>
    </section>
  );
...
function ReviewSection({
  ...
  return (
    <section className="mt-20 rounded-[40px] bg-white p-8 shadow-sm md:p-12">
      <p className="mb-4 text-sm tracking-[0.35em] text-neutral-400">USER EXPERIENCE REVIEW</p>
      <h2 className="mb-4 text-4xl md:text-5xl">사용자 후기</h2>
      ...
      <div className="mb-12 grid gap-6">
        <input
          ...
          className="rounded-[20px] border-2 border-black bg-white px-6 py-5 text-xl"
        />
        ...
        <div className="flex flex-wrap gap-3">
          {[1, 2, 3, 4, 5].map((star) => (
            <button
              ...
              className={`rounded-[18px] border-2 border-black px-6 py-4 text-2xl ${
        ...
        <textarea
          ...
          className="min-h-[160px] rounded-[20px] border-2 border-black bg-white px-6 py-5 text-xl leading-8"
        />

        <button
          onClick={submitReview}
          disabled={loadingReview}
          className="rounded-[24px] border-2 border-black bg-black px-10 py-6 text-2xl font-semibold text-white transition hover:-translate-y-1 disabled:opacity-40"
        >
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {reviews.map((review) => (
          <div key={review.id} className="rounded-[24px] border-2 border-neutral-200 p-6">
            <div className="mb-4 flex items-center justify-between gap-4">
              <p className="text-2xl font-semibold">{review.name || "익명"}</p>
              <p className="text-2xl">{"★".repeat(review.rating)}</p>
            </div>
            <p className="whitespace-pre-wrap text-xl leading-8 text-neutral-700">{review.comment}</p>
