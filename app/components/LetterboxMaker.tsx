...
export default function LetterboxMaker() {
  ...
  function drawMarkerBackground(
    ...
    for (let y = 0; y < height; y += MARKER_TILE) {
      for (let x = 0; x < width; x += MARKER_TILE) {
        const index = (Math.floor(x / MARKER_TILE) + Math.floor(y / MARKER_TILE)) % 2;
  ...
  function drawAddMode(
    ...
    if (bgMode === "blur") {
      const cover = coverRect(item.image.naturalWidth, item.image.naturalHeight, size);
  ...
  function findTrimRectByPredicate(
    ...
    isBorder: (r: number, g: number, b: number, a: number, index: number) => boolean,
    label: string
  ): Rect | null {
    let minX = width;
    let minY = height;
  ...
  function detectAutoTrim(image: HTMLImageElement) {
    return detectMarkerTrim(image) || detectWhiteTrim(image) || detectEdgeTrim(image);
  }
  ...
  async function renderItemToBlob(item: ImageItem) {
    ...
    if (toolMode === "add") {
      drawAddMode(tempCtx, tempCanvas, item);
    } else {
      drawRemoveMode(tempCtx, tempCanvas, item);
    }
  ...
  return (
    <section className="mt-10 rounded-[28px] bg-white p-6 shadow-sm md:p-8">
      ...
      <p className="mb-6 text-lg leading-8 text-neutral-600">
        ...
        편집 도구입니다. 여러 장을 선택하면 ZIP 파일로 한 번에 저장할 수 있습니다.
      </p>
